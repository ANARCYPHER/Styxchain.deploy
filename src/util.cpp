...
#else
    const char* pszModule = "styxchain";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "styxchain.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "styxchaind.pid"));
